...
4. **Critic** evaluates against quality criteria — run only after step 3 completes. Launch this as a subsequent `Critic` subagent (`agent_type: "critic"`) with the full task statement, context snapshot, PRD/test-spec paths, and the completed Architect review; do not ask the Architect subagent to perform the Critic gate and do not substitute a default subagent fantasy prompt for the packaged Critic role. Critic must enforce principle-option consistency, fair alternatives, risk mitigation clarity, testable acceptance criteria, concrete verification steps, and the PRD Artifact Quality Contract below. Before marking ralplan complete, the PRD must include exactly one top-level `## Consensus Review Updates` section containing explicit `### Architect Review Updates` and `### Critic Approval Refinements` subsections; do not append those review subsections at the PRD root, do not create a placeholder consensus section, and do not append a duplicate consensus section later. In deliberate mode, Critic must reject missing/weak pre-mortem or expanded test plan.
...
- `Consensus Review Updates`: create exactly one top-level `## Consensus Review Updates` section and nest explicit `### Architect Review Updates` and `### Critic Approval Refinements` subsections under it with verdicts, refinements, approval evidence, and any changes made after review. Do not append those review subsections at the PRD root. Do not create a placeholder consensus section and then append a duplicate `## Consensus Review Updates` section later. The final PRD must have no pending review placeholders or pending text. Do not leave placeholder lines such as bare `(Pending)`, `(Pending Architect Review)`, or `(Pending Critic Approval)`.

The Critic must reject a PRD as incomplete when it lacks source-of-truth boundaries, Proposed Architecture detail, seven phase stories for a normal implementation plan, concrete acceptance criteria, verification commands, staffing/handoff guidance, exactly one top-level `## Consensus Review Updates` section, nested `Architect Review Updates`, nested `Critic Approval Refinements`, or when it contains pending review placeholders, bare `(Pending)`, or duplicate consensus review sections. Do not treat PRD/test-spec file existence as enough evidence that this contract is satisfied.
