...
<tools>
Use repo inspection for facts, the surface-appropriate structured question path only for real preferences/branches (`omx question` in attached tmux, native structured input when available, plain text only as last fallback), the current runtime's available file-writing surface for plan artifacts, and upward handoff for external research needs.
</tools>
