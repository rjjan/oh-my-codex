...
function buildAdditionalContextMessage(
  ...
  const ralplanPromptActivationNote = skillState?.initialized_mode === "ralplan" || match.skill === "ralplan"
    ? [
      ...
      "PRD Artifact Quality Contract: include source-of-truth and planning boundary, Requirements Summary, RALPLAN-DR Summary, Proposed Architecture, seven phase stories with edge cases and test commands, Implementation Steps, Acceptance Criteria, Risks and Mitigations, Verification Steps, ADR, Staffing / Handoff, and exactly one top-level `## Consensus Review Updates` section. The Consensus Review Updates section must contain exact nested subsections `### Architect Review Updates` and `### Critic Approval Refinements`; do not append those review subsections at the PRD root. Do not write a placeholder Consensus Review Updates section and then append a duplicate `## Consensus Review Updates` section later. The final PRD must have no pending review placeholders or pending text, including bare `(Pending)`, Pending Architect Review, or Pending Critic Approval.",
