...
function buildAdditionalContextMessage(
  ...
  const ralplanPromptActivationNote = skillState?.initialized_mode === "ralplan" || match.skill === "ralplan"
    ? [
      ...
      "PRD Artifact Quality Contract: include source-of-truth and planning boundary, Requirements Summary, RALPLAN-DR Summary, Proposed Architecture, seven phase stories with edge cases and test commands, Implementation Steps, Acceptance Criteria, Risks and Mitigations, Verification Steps, ADR, Staffing / Handoff, and exactly one top-level `## Consensus Review Updates` section. Initial draft rule: do not create `## Consensus Review Updates`, `### Architect Review Updates`, or `### Critic Approval Refinements` before Architect and Critic reviews are complete; omit the entire consensus review section instead of writing placeholders. The final Consensus Review Updates section must contain exact nested subsections `### Architect Review Updates` and `### Critic Approval Refinements`; do not append those review subsections at the PRD root. Do not write a placeholder Consensus Review Updates section and then append a duplicate `## Consensus Review Updates` section later. Before marking ralplan complete, run a cleanup self-check for Pending, duplicate `## Consensus Review Updates`, duplicate `### Architect Review Updates`, and duplicate `### Critic Approval Refinements`; remove stale review headings. The final PRD must have no pending review placeholders or pending text, including bare `(Pending)`, Pending Architect Review, or Pending Critic Approval.",
