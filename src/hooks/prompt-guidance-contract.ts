...
export const PROMPT_REFACTOR_INVARIANT_CONTRACTS: GuidanceSurfaceContract[] = [
  ...
  },
  {
    ...
    requiredPatterns: [
      ...
      rx('Initial PRD draft rule|Initial draft rule'),
      rx('omit.*consensus review section'),
      rx('Consensus Review Updates'),
      rx('top-level.*Consensus Review Updates|## Consensus Review Updates'),
      rx('PRD root'),
      rx('bare.*Pending|Pending.*bare|\\(Pending\\)'),
      rx('cleanup self-check'),
      rx('duplicate.*Consensus Review Updates|Consensus Review Updates.*duplicate'),
      rx('duplicate.*Architect'),
      rx('duplicate.*Critic'),
